<template>
  <div class="auth-page">
    <RouterLink to="/" class="back-link">← main page</RouterLink>
    <div class="container">
      <h1 class="loading-text" v-if="!userName">Processing authorizations...</h1>
  ...
  if (code) {
    fetch('http://localhost:8000/api/auth/google/callback', {
      ...
      .then(data => {
        ...
        // you will receive your access/refresh tokens here — persist them as you want
        console.log("received tokens:", data.access_token, data.refresh_token);
      })
      .catch(err => {
        console.error(err);
        this.message = 'Authorization failed: ' + err.message;
      });
  } else {
    this.message = '⚠️ No code in query params';
  }
}
