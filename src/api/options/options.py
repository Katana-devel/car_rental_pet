...
async def get_options(limit: int = Query(10, ge=1, le=500), offset: int = Query(0, ge=0), db: AsyncSession = Depends(get_db)):
...
async def get_options_by_name(
    ...
    # если прилетела строка через Query
    if isinstance(option_name, str):
        # можно поддержать формат через запятую: ?option_name=a,b,c
